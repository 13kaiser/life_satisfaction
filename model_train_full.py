...
torch.save(checkpoint, 'C:/Arbeit/Paper/Alfonso_Steffe_ML/Otterbach/checkpoint.pth')
...
plt.savefig("baseline.png",dpi=1000)
plt.show()



time=final.groupby("time",as_index=False)["pre","lfsat"].mean()

years=["1996","2001","2006","2011","2016"]
plt.plot(time.time,time.pre, label="predicted values")
plt.plot(time.time,time.lfsat, label="actual values")
plt.ylabel("life Satisfaction")
plt.xlabel("time")
plt.legend(("predicted values", "actual values"))
plt.savefig("baseline.png",dpi=1000)
plt.xticks([5,10,15,20,25],years)
plt.savefig("baseline_cohort.png",dpi=1000)
plt.show()
